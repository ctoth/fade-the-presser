...
skips meetings already scored in index.html, and for each new one:
fetches the minutes (HTML) and the press-conference transcript (PDF),
runs the Claude analysis from analyze.py, and injects the scored meeting
into index.html. Exits 0 quietly when there is nothing new.

Usage:
    python auto_update.py            # full run (needs ANTHROPIC_API_KEY)
    python auto_update.py --dry-run  # fetch + parse only, no API call
...
def scored_ids() -> set[str]:
    match = DATA_RE.search(INDEX.read_text(encoding="utf-8"))
    if not match:
        sys.exit("index.html: meeting-data block not found.")
    data = json.loads(match.group(2))
    return {m["id"] for m in data["meetings"] if m.get("status") == "scored"}


def published_minutes() -> list[str]:
    page = requests.get(CALENDAR, headers=UA, timeout=60)
    page.raise_for_status()
    codes = sorted(set(re.findall(r"fomcminutes(\d{8})\.htm", page.text)))
    return codes
...
def main() -> None:
    ...
    new = 0
    for code in reversed(published_minutes()):
        if code < ERA_START:
            continue
        ...
        if args.dry_run:
            print(f"{meeting_id}: DRY RUN ok — minutes {len(minutes_text):,} chars, "
                  f"presser {len(presser_text):,} chars ({label})")
            new += 1
            continue
